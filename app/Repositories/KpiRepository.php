...
class KpiRepository
{
    ...
    public function firstReplyKpi($agent = null)
    {
        ...
        if (!$agent)
            return $this->toTime($kpi->forType(Kpi::TYPE_USER));
        if ($agent instanceof User)
            return $this->toTime($kpi->forUser(auth()->user()));
    ...
    public function oneTouchResolutionKpi($agent = null)
    {
        ...
        if (!$agent)
            return $this->toPercentage($kpi->forType(Kpi::TYPE_USER));
        if ($agent instanceof User)
            return $this->toPercentage($kpi->forUser(auth()->user()));
    ...
    public function reopenedKpi($agent = null)
    {
        ...
        if (!$agent)
            return $this->toPercentage($kpi->forType(Kpi::TYPE_USER), true);
        if ($agent instanceof User)
            return $this->toPercentage($kpi->forUser(auth()->user()), true);
