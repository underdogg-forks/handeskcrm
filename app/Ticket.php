...
class Ticket extends BaseModel
{
    ...
    public function addComment($user, $body, $newStatus = null)
    {
        ...
        tap(new NewComment($this, $comment), function ($newCommentNotification) {
            ...
            if ($this->user && (!auth()->user() || auth()->user()->id != $this->user->id))
                $this->user->notify($newCommentNotification);
            if ($this->requester && auth()->user())
                $this->requester->notify($newCommentNotification);
